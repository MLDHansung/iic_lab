...
def extract_features(file_name): # MFCC transform 
    """
...
def extract_features_scalogram(file_name): # Scalogram transform 
    """
    This function takes in the path for an audio file as a string, loads it, and returns the MFCC
    of the audio"""
    max_pad_len = 1393
...
for file_name in filepaths:
    ...
    for file_name2 in filepaths_waveform:
        if(file_name.split('/')[-1]+'_waveform.png'==file_name2.split('/')[-1]):
            data = extract_image(file_name2)
            img_features.append(data)
    p_id.append(file_name.split('/')[-1])
p_id = np.array(p_id)
...
scalo_datset = np.transpose(scalo_features1, (0,3,1,2))
waveform_dataset = np.transpose(img_features1, (0,3,1,2))
spt_train, spt_test, label_train, label_test = train_test_split(spt_dataset, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
scalo_train, scalo_test, label2_train, label2_test = train_test_split(scalo_datset, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
waveform_train, waveform_test, label3_train, label3_test = train_test_split(waveform_dataset, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
_, p_id, _, _ = train_test_split(p_id, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
print('wave.shape',waveform_train.shape)
print('scalo_train.shape',scalo_train.shape)
np.save("scalo_train.npy",scalo_train)
np.save("scalo_test.npy",scalo_test)
# construct model on cuda if available
use_cuda = torch.cuda.is_available()
# loss
criterion = nn.MultiLabelSoftMarginLoss()
class spt_cnn(nn.Module): # MFCC CNN Class
...
class scalo_cnn(nn.Module): # Scalogram CNN Class
...
class concat_fc(nn.Module): # Ensemble CNN Class
    def __init__(self):
...
spt_cnn = spt_cnn()
spt_cnn.cuda()
spt_cnn.load_state_dict(torch.load(path+"/weight/symptom_wav_cnn_weight_4_1000_128_1626675124.4169002.pt"))
spt_optimizer = optim.Adam(spt_cnn.parameters(), lr=learning_rate)

scalo_cnn = scalo_cnn()
scalo_cnn.cuda()
scalo_cnn.load_state_dict(torch.load(path+"/weight/symptom_scalo_cnn_weight_4_300_128_1626674999.2049477.pt"))
...
def spt_cnn_test(spt_data): # MFCC CNN test
    spt_cnn.eval()
...
def scalo_cnn_test(scalo_data): # Scalogram CNN test
    scalo_cnn.eval()
...
def concat_test(concat_testset): # Ensemble test
    concat_fc.eval()
...
## CNN model's feature
_, spt_feature = spt_cnn_test(spt_test)
spt_feature = spt_feature.reshape(spt_feature.shape[0],spt_feature.shape[1])
_, scalo_feature = scalo_cnn_test(scalo_test)
_, img_feature = img_cnn_test(waveform_test)
...
concat_pred = np.concatenate((p_id, classpreds,target),1)

df = pd.DataFrame(concat_pred,columns=['p_id','T_Ensem','target'])
count_t = time.time()
df.to_excel(path+"result/symptom_result_{}.xlsx".format(count_t), sheet_name = 'sheet1')