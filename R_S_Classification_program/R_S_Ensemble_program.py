...
filepaths = [join(mypath, f) for f in filenames] # full paths of sound files
filepaths_waveform = [join(mypath_waveform, f) for f in filenames_waveform] # full paths of waveform files
labels = np.array([p_diag[p_diag[0] == x][1].values[0] for x in p_id_in_file]) # label matching
...
scalo_train, scalo_test, _, _ = train_test_split(scalo_dataset, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
waveform_train, waveform_test, _, _ = train_test_split(waveform_dataset, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
...
MFCC_cnn.load_state_dict(torch.load(path+"/weight/MFCC_cnn2_weight_1000_128_1631600870.018815.pt"),strict=False)
...
concat_fc.load_state_dict(torch.load(path+"/weight/ensemble_weight_300_64_1638270732.244066.pt"),strict=False)
...
def concat_train(mfcc_trainset,scalogram_trainset,waveform_trainset,feature_label):
    ...
    count_t = time.time()
    torch.save(concat_fc.state_dict(), path+"/weight/ensemble_weight_{}_{}_{}.pt".format(ensem_num_epochs, batch_size, count_t))
...
ensem_predictions = concat_test(mfcc_test, scalo_test, waveform_test)
