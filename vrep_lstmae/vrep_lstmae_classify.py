...
#create sequence
def sliding_windows(train_data, data, seq_length):
    x = []
    y = []
    print('train_data_sw', train_data.shape)
    for i in range(1, len(train_data) - seq_length - 1):
        _x = train_data[i:(i + seq_length - 1)]
...
'''def evaluate_confidence(bsz, output_dim, input_dim, hidden_size, numlayers, dropout_prob_in, dropout_prob,
    ...
    for jj in range(num_evals):
        ...
        for ii in range(int((dataX.shape[1] / bsz))):
            ...
            predictions[jj, :, (ii * bsz):((ii + 1) * bsz), :] = output.cpu().data.numpy()  # 모든값 넣기
            tmp = loss_function_test(output, input_r)
...
train_loss = train(trainX, epoch, num_epochs, learning_rate, bsz, output_dim, input_dim, hidden_size, numlayers,
