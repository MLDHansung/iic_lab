...
train_test_dataset = dataset.filter(
    items=['joint1', 'joint2','joint1_difference','joint2_difference'])
train_test_dataset_unnormalized = dataset.filter(
    items=['path', 'seqNum'])
label_set = dataset.filter(items=['class'])
le = LabelEncoder()
training_set = train_test_dataset.iloc[:, :]
training_set_unnoramlized = train_test_dataset_unnormalized.iloc[:, :]
train_label_set = label_set[:]

sc = MaxAbsScaler(1)
training_set = sc.fit_transform(training_set)
training_set_unnoramlized = np.array(training_set_unnoramlized)  
#training_set = np.concatenate((training_set_unnoramlized,training_set),axis=1)
...
train_test_dataset2 = dataset2.filter(
    items=['joint1', 'joint2','joint1_difference','joint2_difference'])
train_test_dataset_unnormalized2 = dataset2.filter(
    items=['path', 'seqNum'])
...
training_set_unnoramlized2 = train_test_dataset_unnormalized2.iloc[:, :]
training_set2 = sc.fit_transform(training_set2)
training_set_unnoramlized2 = np.array(training_set_unnoramlized2)  
#training_set2 = np.concatenate((training_set_unnoramlized2,training_set2),axis=1)
...
training_set2 = training_set2_temp[:iii+ii+1,:]
train_label_Y2 = train_label_Y2_temp[:iii+ii+1]
...
train_test_dataset3 = dataset3.filter(
    items=['joint1', 'joint2','joint1_difference','joint2_difference'])
train_test_dataset_unnormalized3 = dataset3.filter(
    items=['path', 'seqNum'])
label_set3 = dataset3.filter(items=['class'])
train_label_set3 = label_set3[:]
training_set3 = train_test_dataset3.iloc[:, :]
training_set_unnormalized3 = train_test_dataset_unnormalized3.iloc[:, :]
training_set3 = sc.fit_transform(training_set3)
training_set_unnormalized3 = np.array(training_set_unnormalized3)  
#training_set3 = np.concatenate((training_set_unnormalized3,training_set3),axis=1)
...
def train(train_data,train_label):
    ...
    torch.save(seq.state_dict(), "/home/iichsk/workspace/vrep_lstmae/weight/vrep_lstm_weight_{}_{}_{}_{}.pt".format(numlayers, num_epochs, bsz, count_t))
