...
def checkpoint2():
    ...
    io.savemat(
        ...
        {'normaljoint2test': predict1_normal,'normaljoint2target': normal_inverse,
        'joint2errorpredict': predict2_inverse, 'joint2target': testX2_inverse, 
        'MSE{}': eval_loss2, 'trainloss': train_loss, })
...
trainX, testX=lap.load_data(path, seq_len)
...
predict1, eval_loss = evaluate(testX, test_bsz, output_dim, input_dim, hidden_size, numlayers,
                                                     dropout_prob_in, dropout_prob, dropout_prob_fc)
predict1_normal, normal_inverse = lap.normalize_inverse(predict1, testX)
print('normal data test loss = ', eval_loss.mean())

#test start
print('start test2....')
test_bsz=1
path2='/home/iichsk/workspace/dataset/vrep_dataset/vrep_joint2_bsf_01.csv'
_, testX2=lap.load_data(path2, seq_len)
print('testX2.shape', testX2.shape)
predict2, eval_loss2 = evaluate(testX2, test_bsz, output_dim, input_dim, hidden_size, numlayers,
                                                     dropout_prob_in, dropout_prob, dropout_prob_fc)
print('error data test loss = ', eval_loss2.mean())

predict2_inverse, testX2_inverse = lap.normalize_inverse(predict2, testX2)
