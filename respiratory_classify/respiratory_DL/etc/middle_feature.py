...
class concat_fc(nn.Module):
    ...
    def forward(self, concat_x):
        ...
        concat_feature2 = concat_x
        concat_x = self.concat_fc2(concat_x)  #
        

        return concat_x, concat_feature, concat_feature2
...
def concat_train(concat_trainset,feature_label):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(int(concat_trainset.shape[0] / batch_size)):
            ...
            model_output,_,_ = concat_fc(concat_input)
            # calculate loss
...
def concat_test(concat_testset):
    ...
    concat_feature = np.zeros((int(concat_testset.shape[0]), 384))
    concat_feature2 = np.zeros((int(concat_testset.shape[0]), 128))
    ...
    for j in range(int(concat_testset.shape[0])):
        ...
        test_output, feature1, feature2 = concat_fc(concat_input)
        predictions[j:j+1,:] = test_output.detach().cpu().numpy()
        concat_feature[j:j+1,:] = feature1.detach().cpu().numpy()
        concat_feature2[j:j+1,:] = feature2.detach().cpu().numpy()
        #target[j:j+1, :] = test_label.detach().cpu().numpy()
        #pred = test_output.data.max(1)[1]
        #one_hot_pred = torch.zeros(1, classes)
        #one_hot_pred[range(one_hot_pred.shape[0]), pred]=1
    ...
    #print(" Error: {}/{} ({:.2f}%)".format(wrong,wrong+correct, 100.*(float(wrong)/float(correct+wrong))))
    #print("eval_prediction.shape:", predictions.shape)
    return predictions, concat_feature, concat_feature2
...
def imshow(img,name,text=None):
    ...
    plt.savefig('/home/iichsk/workspace/wavpreprocess/{}.png'.format(name))    
    plt.close()
...
print('spt_feature',spt_feature.shape)
spts=np.squeeze(spt_feature)
spts=np.expand_dims(spts,axis=1)
print('spts',spts)
for i in range(int(spts.shape[0])):
    imshow(torchvision.utils.make_grid(torch.from_numpy(spts[i,:,:])),'mfcc_{}'.format(i))
...
imgs=np.squeeze(img_feature)
imgs=np.expand_dims(imgs,axis=1)
print('imgs',imgs)
for i in range(int(imgs.shape[0])):
    imshow(torchvision.utils.make_grid(torch.from_numpy(imgs[i,:,:])),'waveform_{}'.format(i))
...
predictions, concat_feature, concat_feature2 = concat_test(concat_testset)
con_pred=np.squeeze(concat_feature)
con_pred=np.expand_dims(con_pred,axis=1)
con_pred2=np.squeeze(concat_feature2)
con_pred2=np.expand_dims(con_pred2,axis=1)
for i in range(int(con_pred.shape[0])):
    imshow(torchvision.utils.make_grid(torch.from_numpy(con_pred[i,:,:])),'ensem_{}'.format(i))
    imshow(torchvision.utils.make_grid(torch.from_numpy(con_pred2[i,:,:])),'ensem_fc_{}'.format(i))
