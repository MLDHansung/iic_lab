...
class concat_fc(nn.Module):
    def __init__(self):
        ...
        self.concat_conv1 = nn.Conv1d(in_channels=384,out_channels=512,kernel_size=1)
        self.concat_conv1_bn = torch.nn.BatchNorm1d(512)

        self.concat_conv2 = nn.Conv1d(in_channels=512,out_channels=512,kernel_size=1)
        self.concat_conv2_bn = torch.nn.BatchNorm1d(512)

        self.concat_conv3 = nn.Conv1d(in_channels=512,out_channels=1024,kernel_size=1)
        self.concat_conv3_bn = torch.nn.BatchNorm1d(1024)

        self.concat_conv4 = nn.Conv1d(in_channels=1024,out_channels=1024,kernel_size=1)
        self.concat_conv4_bn = torch.nn.BatchNorm1d(1024)

        # self.concat_conv5 = nn.Conv1d(in_channels=512,out_channels=1024,kernel_size=1)
        # self.concat_conv5_bn = torch.nn.BatchNorm1d(1024)


        self.concat_fc1 = nn.Linear(1024, 512)
        self.concat_fc2 = nn.Linear(512, classes)
        self.concat_fc3 = nn.Linear(128, classes)
        self.concat_relu = nn.ReLU()
        self.concat_fc1_bn = torch.nn.BatchNorm1d(512)
        self.concat_fc2_bn = torch.nn.BatchNorm1d(128)
    ...
    def forward(self, concat_x):   
        ...
        concat_x = self.concat_conv4(concat_x)
        concat_x = self.concat_conv4_bn(concat_x)
        concat_x = self.concat_dropout(concat_x)
        concat_x = self.concat_relu(concat_x)

        # concat_x = self.concat_conv5(concat_x)
        # concat_x = self.concat_conv5_bn(concat_x)
        # concat_x = self.concat_dropout(concat_x)
        # concat_x = self.concat_relu(concat_x)
        ...
        #concat_x = self.concat_fc2_bn(concat_x)
        #concat_x = self.concat_dropout(concat_x)
        #concat_x = self.concat_relu(concat_x)
...
concat_fc = concat_fc()
concat_fc.cuda()
concat_fc.load_state_dict(torch.load("/home/iichsk/workspace/respiratory_classify/respiratory_DL/weight/concat_fc_weight_550_128_1628490994.0059733.pt"))
...
# np.save('/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_data/img_feature2',img_feature)
img_feature_x = np.load('/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_data/img_feature_x.npy')
img_feature = np.load('/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_data/img_feature.npy')
...
classpreds = triple_correct(classpreds)
c_names = ['normal','wheezes','crackles','Both']
# Classification Report
...
df = pd.DataFrame(concat_pred,columns=['concat','target'])
count_t = time.time()
