...
for file_name in filepaths:
    ...
    p_id.append(file_name)
p_id = np.array(p_id)
# p_id=pd.DataFrame(p_id)
# p_id.to_csv('best_p_id.csv')
...
# scalo_test = np.transpose(scalo_features1, (0,3,1,2))
waveform_test = np.transpose(img_features1, (0,3,1,2))
# np.save('/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_data/scalo_test',scalo_test)
scalo_test = np.load('/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_data/scalo_test.npy')
spt_train, spt_test, label_train, label_test = train_test_split(spt_test, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
scalo_train, scalo_test, label2_train, label2_test = train_test_split(scalo_test, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
waveform_train, waveform_test, label3_train, label3_test = train_test_split(waveform_test, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
_, p_id, _, _ = train_test_split(p_id, oh_labels, stratify=i_labels,
                                                    test_size=0.2, random_state = 42)
...
concat_fc = concat_fc()
concat_fc.cuda()
concat_fc.load_state_dict(torch.load("/home/iichsk/workspace/weight/concat_fc_weight_350_128_1631882336.1835415.pt"))
...
spt_prediction = np.argmax(spt_prediction, axis=1)  # predicted classes

c_names = ['normal','wheezes','crackles','Both']
# Classification Report
print('#'*10,'MFCC CNN report','#'*10)
print(classification_report(target, spt_prediction, target_names=c_names))
# Confusion Matrix
print(confusion_matrix(target, spt_prediction))

img_prediction = np.argmax(img_prediction, axis=1)  # predicted classes

c_names = ['normal','wheezes','crackles','Both']
# Classification Report
print('#'*10,'Waveform CNN report','#'*10)
print(classification_report(target, img_prediction, target_names=c_names))
# Confusion Matrix
print(confusion_matrix(target, img_prediction))

scalo_prediction = np.argmax(scalo_prediction, axis=1)  # predicted classes

c_names = ['normal','wheezes','crackles','Both']
# Classification Report
print('#'*10,'Scalogram CNN report','#'*10)
print(classification_report(target, scalo_prediction, target_names=c_names))
# Confusion Matrix
print(confusion_matrix(target, scalo_prediction))
...
spt_prediction=np.expand_dims(spt_prediction, axis=1)
img_prediction=np.expand_dims(img_prediction, axis=1)
scalo_prediction=np.expand_dims(scalo_prediction, axis=1)

classpreds2=np.expand_dims(classpreds, axis=1)
target=np.expand_dims(target, axis=1)
p_id = np.expand_dims(p_id, axis=1)
print('p_id=',p_id.shape)
print('classpreds=',classpreds.shape)
print('target=',target.shape)
# concat_pred = np.concatenate((p_id,spt_prediction,img_prediction,scalo_prediction, classpreds,target),1)

df = pd.DataFrame(classpreds)
count_t = time.time()
df.to_excel("/home/iichsk/workspace/wavpreprocess/classpred_result_{}.xlsx".format(count_t), sheet_name = 'sheet1')
