...
class concat_fc(nn.Module):
    ...
    def forward(self, concat_x):   
        ...
        return concat_x, concat_feature
...
img_cnn = img_cnn()
img_cnn.cuda()
img_cnn.load_state_dict(torch.load("/home/iichsk/workspace/weight/sym_wave/symptom_wav_cnn_weight_4_1000_128_1630997427.7513587.pt"))
...
def concat_train(concat_trainset,feature_label):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(int(concat_trainset.shape[0] / batch_size)):
            ...
            model_output,_ = concat_fc(concat_input)
            # calculate loss
...
def concat_test(concat_testset):
    ...
    concat_features = np.zeros((int(concat_testset.shape[0]), 384))
    ...
    for j in range(int(concat_testset.shape[0])):
        ...
        test_output, concat_feature = concat_fc(concat_input)

        predictions[j:j+1,:] = test_output.detach().cpu().numpy()
        concat_features[j:j+1,:] = concat_feature.detach().cpu().numpy()

    return predictions,concat_features
...
spt_feature_x = np.load('/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_data/spt_feature_x_norm.npy')
...
##############train################################
spt_scaling_result = []
scalo_scaling_result = []
wave_scaling_result = []
print('1spt_feature_x=',spt_feature_x.shape)
print('1scalo_feature_x=',scalo_feature_x.shape)
print('1img_feature_x=',img_feature_x.shape)
spt_feature_x=sc.fit_transform(spt_feature_x)
scalo_feature_x=sc.fit_transform(scalo_feature_x)
img_feature_x=sc.fit_transform(img_feature_x)
print('2spt_feature_x=',spt_feature_x.shape)
print('2scalo_feature_x=',scalo_feature_x.shape)
print('2img_feature_x=',img_feature_x.shape)
...
concat_trainset = np.concatenate((spt_feature_x, scalo_feature_x, img_feature_x),1)
...
print('1spt_feature=',spt_feature.shape)
print('1scalo_feature=',scalo_feature.shape)
print('1img_feature=',img_feature.shape)
spt_feature=sc.fit_transform(spt_feature)
scalo_feature=sc.fit_transform(scalo_feature)
img_feature=sc.fit_transform(img_feature)
print('2spt_feature=',spt_feature.shape)
print('2scalo_feature=',scalo_feature.shape)
print('2img_feature=',img_feature.shape)
...
concat_testset = np.concatenate((spt_feature, scalo_feature, img_feature),1)
# concat_testset = np.reshape(concat_testset, (*concat_testset.shape, 1))
# concat_testset = np.transpose(concat_testset, (0,3,1,2))

predictions, concat_feature = concat_test(concat_testset)
...
scalo_feature = np.squeeze(scalo_feature)
concat_feature = np.squeeze(concat_feature)
...
count_t = time.time()
df2.to_excel("/home/iichsk/workspace/wavpreprocess/pre_mfcc_feature_{}.xlsx".format(count_t), sheet_name = 'sheet1')

df3 = pd.DataFrame(scalo_feature)
count_t = time.time()
df3.to_excel("/home/iichsk/workspace/wavpreprocess/pre_scalo_feature_{}.xlsx".format(count_t), sheet_name = 'sheet1')

df4 = pd.DataFrame(concat_feature)
count_t = time.time()
df4.to_excel("/home/iichsk/workspace/wavpreprocess/pre_concat_feature_{}.xlsx".format(count_t), sheet_name = 'sheet1')
# for i in range(int(spt_feature.shape[0])):
    
    
#     spt_feature_im = Image.fromarray(spt_feature[i,:,:])
#     spt_feature_im.save("/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_image/spt_feature_{}.tiff".format(i))
#     scalo_feature_im = Image.fromarray(scalo_feature[i,:,:])
#     scalo_feature_im.save("/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_image/scalo_feature_{}.tiff".format(i))
#     concat_feature_im = Image.fromarray(concat_feature[i,:])
#     concat_feature_im.save("/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_image/feature_{}.tiff".format(i))
#     concat_testset_im = Image.fromarray(concat_testset[i,:])
#     concat_testset_im.save("/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/feature_image/testset_feature_{}.tiff".format(i))


#img_feature = img_feature.reshape(img_feature.shape[0],img_feature.shape[1])
