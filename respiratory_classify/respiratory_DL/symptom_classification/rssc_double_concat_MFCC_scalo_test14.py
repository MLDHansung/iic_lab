...
def concat_train(concat_trainset,feature_label):
    ...
    count_t = time.time()
    torch.save(concat_fc.state_dict(), "/home/iichsk/workspace/respiratory_classify/respiratory_DL/weight/concat_fc_weight_{}_{}_{}.pt".format(num_epochs, batch_size, count_t))
...
concat_trainset = np.concatenate((spt_feature_x, scalo_feature_x),1)
...
concat_testset = np.concatenate((spt_feature, scalo_feature,),1)
...
df = pd.DataFrame(concat_pred,columns=['MFCC','Scalo','concat','target'])
count_t = time.time()
df.to_excel("/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/double_predict_value_{}.xlsx".format(count_t), sheet_name = 'sheet1')
