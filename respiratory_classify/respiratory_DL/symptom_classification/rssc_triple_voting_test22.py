...
class concat_fc(nn.Module):
    ...
    def forward(self, spt_input, scalo_input, img_input):   
        spt_input = self.concat_fc1_bn(spt_input)
        spt_bn = spt_input

        scalo_input = self.concat_fc1_bn(scalo_input)
        scalo_bn = scalo_input

        img_input = self.concat_fc1_bn(img_input)
        img_bn = img_input

        concat_x = torch.cat((spt_input, scalo_input, img_input),2)
        # print('1concat_x',concat_x.shape)
        concat_x = torch.reshape(concat_x, (*concat_x.shape, 1))
        # print('2concat_x',concat_x.shape)
        concat_x = concat_x.permute(0,3,1,2)
        # print('3concat_x',concat_x.shape)
        ...
        return concat_x, concat_feature, spt_input, scalo_input, img_input
...
def concat_test(spt_feature, scalo_feature, img_feature):
    ...
    mfcc_features = np.zeros((int(spt_feature.shape[0]), 128,1))
    scalogram_features = np.zeros((int(spt_feature.shape[0]), 128,1))
    waveform_features = np.zeros((int(spt_feature.shape[0]), 128,1))
...
count_t = time.time()
df2.to_excel("/home/iichsk/workspace/wavpreprocess/mfcc_feature_bn{}.xlsx".format(count_t), sheet_name = 'sheet1')

df3 = pd.DataFrame(scalogram_features)
count_t = time.time()
df3.to_excel("/home/iichsk/workspace/wavpreprocess/scalo_feature_bn{}.xlsx".format(count_t), sheet_name = 'sheet1')

df4 = pd.DataFrame(waveform_features)
count_t = time.time()
df4.to_excel("/home/iichsk/workspace/wavpreprocess/waveform_features_bn{}.xlsx".format(count_t), sheet_name = 'sheet1')
