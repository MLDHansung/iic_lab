...
mfcc_testset = np.load('mfcc_normal_d.npy')
test_label = np.load('dataset_label_normal_d.npy')
...
MFCC_cnn.load_state_dict(torch.load("/home/iichsk/workspace/weight/MFCC_normal_200.pt"))
...
mdict = {0:'o',1:'v',2:'x',3:'^',4:'*',5:'s'}

labeldict= {0:'normal_Bronchiectasis',1:'normal_Bronchiolitis',2:'normal_COPD',3:'normal_Pneumonia',4:'normal_URTI',5:'normal_Healthy'}


for g in np.unique(group):
    i=np.where(group == g)
    ax.scatter(embedded[i,0],embedded[i,1],embedded[i,2],s=10,marker=mdict[g],label=labeldict[g])
ax.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
for x in range(0,361,10):
    for y in range(0,361,10):
        ax.view_init(x,y)
        plt.savefig('/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/5classes/T-SNE_3D_image_normal/MFCC_feature_T_sne_{}_{}.png'.format(x,y),bbox_inches='tight')
