...
p_diag = pd.read_csv("/home/iichsk/workspace/dataset/Respiratory_Sound_Database/Respiratory_Sound_Database/csv_data/symptom_label_normal.csv",header=None) # patient diagnosis csv file
p_diag_false = pd.read_csv("/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/5classes/pidinfile.csv",header=None) # patient diagnosis csv file
...
for x in p_diag[0]:
    for name in filenames:
        if(int(x) == int(name.split('_')[0])):
            # if(p_diag_false.iloc[cnt,2]==False):
            #     print('cnt2=',cnt2)
            #     p_diag.iloc[cnt2,1] = 'mix'
            cnt+=1
...
df = pd.DataFrame(p_diag)
count_t = time.time()
df.to_excel("/home/iichsk/workspace/respiratory_classify/respiratory_DL/symptom_classification/5classes/p_diag_sd{}.xlsx".format(count_t), sheet_name = 'sheet1')
...
np.save('mfcc_testset_all.npy',mfcc_testset)
np.save('test_label_all.npy',test_label)
...
# tsne_proj = tsne.fit_transform(MFCC_test_features)
# cmap = cm.get_cmap('tab20')
# fig, ax = plt.subplots(figsize=(8,8))

# num_categories = 5
# for lab in range(num_categories):
#     indices = MFCC_predictions_arg == lab
...
# plt.savefig('MFCC_feature_T_sne.png')