...
class spt_cnn(nn.Module):
    ...
    def __init__(self):
        ...
        self.spt_conv4 = nn.Conv2d(64, 128, kernel_size=2)  # 
        self.spt_pool4 = nn.MaxPool2d(2)  # 
        self.spt_dropout4 = nn.Dropout(dropout)
        self.spt_conv5 = nn.Conv2d(128, 256, kernel_size=1)  # 
...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(int(spt_train.shape[0] / batch_size)):
            ...
            # grad init
            optimizer.zero_grad()
            model_output, feature_x = spt_cnn(spt_input)
            # calculate loss
...
def test(spt_test):
    ...
    for j in range(int(spt_test.shape[0])):
        ...
        test_output, feature = spt_cnn(spt_input)
        feature = np.squeeze(feature)
        predictions[j:j+1,:] = test_output.detach().cpu().numpy()
        mfcc_features[j:j+1,:] = feature.detach().cpu().numpy()
        
        
    return predictions, mfcc_features


# train()
print('train finished!!!')
# _, mfcc_features_x = test(spt_train)
...
predictions, mfcc_features = test(spt_test)
