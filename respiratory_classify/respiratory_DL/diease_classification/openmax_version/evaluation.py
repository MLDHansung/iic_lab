...
class Evaluation(object):
    ...
    def __init__(self, predict, label,prediction_scores = None):
        ...
        self.label = label
        self.target = np.argmax(label,axis=1)
    ...
    def _accuracy(self) -> float:
        ...
        :return: float
        """
        print('openmax_predictions=',self.predict)
        print('target class=', self.target)
        assert len(self.predict) == len(self.label)
        correct = (np.array(self.predict) == np.array(self.target)).sum()
    ...
    def _f1_measure(self) -> float:
        ...
        print('')
        assert len(self.predict) == len(self.label)
        return f1_score(self.target, self.predict, average='micro')
    ...
    def _f1_macro(self) -> float:
        ...
        """
        assert len(self.predict) == len(self.label)
        return f1_score(self.target, self.predict, average='macro')
    ...
    def _f1_macro_weighted(self) -> float:
        ...
        """
        assert len(self.predict) == len(self.label)
        return f1_score(self.target, self.predict, average='weighted')
    ...
    def _precision_recall(self, average) -> (float, float):
        ...
        """
        assert len(self.predict) == len(self.label)
        precision, recall, _, _ = precision_recall_fscore_support(self.target, self.predict, average=average)
    ...
    #     """
    #     label, predict = self.target, self.predict
    ...
    def _confusion_matrix(self, normalize=None) -> np.array:
        ...
        """
        assert len(self.predict) == len(self.label)
        return confusion_matrix(self.target, self.predict, normalize=normalize)
