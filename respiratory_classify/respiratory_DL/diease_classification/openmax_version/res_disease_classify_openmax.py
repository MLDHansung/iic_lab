...
# train_dataset = TensorDataset(spt_train, label_train)
...
def spt_cnn_test2(spt_data,spt_train):
    ...
    df_score_openmax.to_excel('/home/iichsk/workspace/result/spt_score_openmax.xlsx')
    df_score_softmax = pd.DataFrame.from_records(score_softmax)
    df_score_softmax.to_excel('/home/iichsk/workspace/result/spt_score_softmax.xlsx')
...
def img_cnn_test2(img_data,img_train):
    ...
    df_score_openmax = pd.DataFrame.from_records(score_openmax)
    df_score_openmax.to_excel('/home/iichsk/workspace/result/img_score_openmax.xlsx')
    df_score_softmax = pd.DataFrame.from_records(score_softmax)
    df_score_softmax.to_excel('/home/iichsk/workspace/result/img_score_softmax.xlsx')
    #print('pred_openmax=',pred_openmax)
    # df_argmax_openmax = pd.DataFrame.from_records(argmax_openmax)
    # df_argmax_openmax.to_excel('/home/iichsk/workspace/result/spt_argmax_openmax.xlsx')
    # df_pred_openmax = pd.DataFrame.from_records(pred_openmax)
...
def concat_test(concat_testset,concat_trainset):
    ...
    for score in scores:
        ...
        score_openmax.append(so)
    df_score_openmax = pd.DataFrame.from_records(score_openmax)
    df_score_openmax.to_excel('/home/iichsk/workspace/result/concat_score_openmax.xlsx')
    df_score_softmax = pd.DataFrame.from_records(score_softmax)
    df_score_softmax.to_excel('/home/iichsk/workspace/result/concat_score_softmax.xlsx')
...
print('pred_openmax2=',pred_openmax)
...
# df=pd.DataFrame.from_records(classpreds[:])
# df.to_excel('classification_result.xlsx')
label_names = ['Bronchiectasis', 'Bronchiolitis', 'COPD', 'Pneumonia', 'URTI', 'Healthy', 'unknown']
#pred_openmax.plot_confusion_matrix(normalize="true",labels=label_names)
print(classification_report(target, pred_openmax, target_names=label_names))

#print('openmax_resultd',eval_openmax)
# df2=pd.DataFrame.from_records(eval_openmax[:])
# df2.to_excel('openmax_result.xlsx')
# df3=pd.DataFrame.from_records(target[:])
# df3.to_excel('label_result.xlsx')
#normal error test
