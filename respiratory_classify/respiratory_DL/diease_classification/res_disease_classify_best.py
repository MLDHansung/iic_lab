...
class concat_fc(nn.Module):
    ...
    def forward(self, concat_x):
        ...
        print('concat_x1',concat_x.shape)
        concat_x = concat_x.view(concat_x.size(0), -1)
        print('concat_x2',concat_x.shape)
        concat_x = self.concat_fc1(concat_x)  #
        print('concat_x_fc1',concat_x.shape)
...
# spt_input = torch.Tensor(spt_test[0:1, :, :, :]).cuda()
# make_dot(spt_cnn(spt_input), params=dict(spt_cnn.named_parameters())).render("graph", format="png")
# print('spt_global_pool.weight:',params(spt_global_pool))
